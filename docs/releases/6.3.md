...
The global function `window.buildExpandingFormset` to attach basic JavaScript behavior for Django's expanding formsets has been deprecated and will be removed in a future release.

Using `InlinePanel` will not be impacted, this only impacts a small set of basic expanding formsets in use across Workflow and Group view editing.

Previously these expanding formsets required a mix of specific id attribute structures and inline scripts to instantiate with callbacks for handling deletion.

In the future, the following data attributes are required to emulate the same behavior, these are likely to change and should not be considered official documentation.
