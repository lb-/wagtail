 ...
 * Rework Page add and create views into class based views with overridable methods leveraging Django's ``TemplateView`` (Matt Westcott, Kate Edmundson)
 * Support custom add/edit page views on a per-page-type basis via ``register_create_page_view`` and ``register_edit_page_view`` (Matt Westcott, Kate Edmundson)
