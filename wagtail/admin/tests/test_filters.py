    ...
    def test_with_relation(self):
        ...
        expected_html = """
            <select name="users" required id="id_users">
                ...
                <option value="{david}" data-w-rules="{musicians_and_actors}" data-w-rules-target="enable show">{david_username}</option>
                <option value="{kevin}" data-w-rules="{actors}" data-w-rules-target="enable show">{kevin_username}</option>
                <option value="{morten}" data-w-rules="{musicians}" data-w-rules-target="enable show">{morten_username}</option>
            </select>
        """.format(
            david=self.david.pk,
            kevin=self.kevin.pk,
            morten=self.morten.pk,
            musicians=escape(json.dumps({"group": ["", str(self.musicians.pk)]})),
            actors=escape(json.dumps({"group": ["", str(self.actors.pk)]})),
            musicians_and_actors=escape(
                json.dumps({"group": ["", str(self.musicians.pk), str(self.actors.pk)]})
            ),
