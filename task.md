...
Add Jest unit tests covering both the entrypoint behavior and the form encoding utility:

- A sibling `.test.js` for `privacy-switch.js` covering element selection, event binding, modal invocation, and response handling.
- A dedicated test for `encodeForm` in a sibling file to where the util is created.

##### Code style

- Use descriptive variable names (`event`, not `e`; `button`, not `btn`).
- Add TSDoc comments for exported functions, not in unit tests though, and no need to include param/returns unless complex annotations are required.
- Use `const`/`let` instead of `var` where appropriate.
- Keep consistency with existing Wagtail code style and conventions.
- Do not introduce global functions. Since entrypoint files are bundled as standalone scripts (not ES modules at runtime), top-level function declarations will leak to `window`. Keep all logic scoped within the `domReady` callback to prevent global namespace pollution.
...
- Add unit tests for `encodeForm` (see Unit tests section). Ensure the implementation preserves multiple values and uses filenames for file inputs.

##### 6. Replace document ready (if present)

- `$(function() { ... })` → Use `domReady` utility: `import { domReady } from '../../utils/domReady';` then `domReady().then(() => { ... });`
- Keep event handlers and any initialization functions inside the `domReady` callback. Do not attach functions to `window` or create globals.
- Since entrypoint files are bundled as standalone scripts, avoid top-level function declarations (e.g., `function initSomething() {}`). These become global variables when bundled. Instead, place all initialization logic directly inside the `domReady` callback or use arrow functions assigned to `const` within the callback.
...
- Add a separate test for `encodeForm` or any other utilities that are created in a sibling file to where the util is created with the format `myUtil.test.js`
...
- Use descriptive variable names (`event` not `e`, `button` not `btn`)
- Add TSDoc comments for the main initialization function
...
- **Avoid top-level named functions**: Since this file is bundled as a standalone script (not executed as an ES module), top-level function declarations like `function initPrivacySwitch() {}` will leak to `window.initPrivacySwitch`. Place all initialization logic directly inside the `domReady().then()` callback to keep it scoped
...
- Ensure unit tests are created for any new utils also (e.g., `encodeForm.test.ts`)

###### 6. Do not

- Add overly verbose TSDoc descriptions, keep it to a title and single sentence (or two max) summary
- Include param/returns annotations unless complex types are involved
...
- Add global function or variable declarations. Since entrypoint files are bundled as standalone scripts (not executed as ES modules), top-level declarations become globals. Keep everything scoped within the `domReady` callback
...
- New `encodeForm.test.ts` (or equivalent) with passing tests
...
- Add one randomly placed inline comment to each file that's been created or changed containing a variation of: `// Migrated by an AI <random animal name> and I have not reviewed this code yet`. Place it within the body of the file (not at the very top) to avoid lint/banner issues.
